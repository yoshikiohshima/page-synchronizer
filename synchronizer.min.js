/* globals Croquet */

/*
Copyright 2023 Yoshiki Ohshima

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

let scrollTarget;

class SimplePageModel extends Croquet.Model {
    init() {
        this.location = null;
        this.owner = null;
        this.subscribe(this.sessionId, "view-exit", this.tryRelease);
        this.subscribe(this.sessionId, "view-join", this.viewJoin);

        this.subscribe(this.id, "position", this.position);
        this.subscribe(this.id, "tryRelease", this.tryRelease);
        this.subscribe(this.id, "tryAcquire", this.tryAcquire);
    }

    tryAcquire(viewId) {
        // console.log("tryAcquire", viewId);
        if (this.owner) {return;}
        this.owner = viewId;
    }

    tryRelease(viewId) {
        // console.log("tryRelease", viewId);
        if (this.owner === viewId) {
            this.owner = null;
        }
    }

    viewJoin() {
        // console.log("viewJoin");
        this.owner = null;
    }

    position(data) {
        if (this.owner && data.viewId !== this.owner) {return;}
        this.location = data; // {ratio, viewId}
        this.publish(this.id, "setPosition", data);
    }
}

SimplePageModel.register("SimplePageModel");

class SimplePageView extends Croquet.View {
    constructor(model) {
        super(model);
        this.model = model;
        this.subscribe(this.model.id, "setPosition", this.setPosition);
        this.subscribe(this.viewId, "synced", this.synced);

        this.ratio = 0; // where this view thinks where my scroll position ratio  is
        this.receivedRatio = 0; // where the last received ratio;
        this.lastSetTime = 0;
        this.lastPublishTime = Date.now();
        this.toBePublished = null;
        this.lastPublished = []; // [{time:number, ratio: number}]

        this.handler = evt => this.userScroll(evt);
        window.document.onscroll = this.handler;
        if (this.model.location) {
            this.setPosition(this.model.location);
        }
    }

    detach() {
        super.detach();
        window.document.onscroll = null;
    }

    initCroquetMessenger() {
        Croquet.Messenger.setReceiver(this);
        if (window.parent !== window) {
            Croquet.Messenger.startPublishingPointerMove();
        }
        Croquet.Messenger.on("userCursor", "handleUserCursor");
        Croquet.Messenger.send("userCursorRequest");
    }

    getTop() {
        if (scrollTarget === document) {
            let top = document.body.getBoundingClientRect().top;
            return -top;
        }
        return scrollTarget.scrollTop;
    }

    setTop(top) {
        if (scrollTarget === document) {
            window.scrollTo(0, top);
        }
        scrollTarget.scrollTop = top;
    }

    getHeight() {
        if (scrollTarget === document) {
            return document.body.getBoundingClientRect().height;
        }
        return scrollTarget.scrollHeight;
    }

    synced(data) {
        if (data) {
            let body = scrollTarget;
            let ratio = this.getTop(body) / this.getHeight(body);
            this.ratio = ratio;
            if (this.model.location) {
                this.lastPublishTime = Date.now();
                this.setPosition(this.model.location);
            }
            console.log("synced", ratio);
        }
    }

    setPosition(data) {
        let {ratio, viewId} = data;
        if (viewId === this.viewId) {return;}
        let body = scrollTarget;
        this.setTop(Math.floor(this.getHeight(body) * ratio));
        this.receivedRatio = ratio;
        this.lastSetTime = Date.now();
        this.lastRatio = ratio;

        // console.log("set", ratio, body.scrollTop, this.lastSetTime);
    }

    resetTimeout() {
        if (this.timeout) {
            clearTimeout(this.timeout);
        }

        this.timeout = setTimeout(() => {
            console.log("reset");
            if (this.lastPublished.findIndex(v => v < 0.01) >= 0) {
                this.publish(this.model.id, "position", {ratio: 0, viewId: this.viewId});
                this.lastPublished = [];
            }
            this.publish(this.model.id, "tryRelease", this.viewId);
        }, 500);
    }

    userScroll(evt) {
        this.resetTimeout();
        this.scroll(evt);
    }

    scroll(_evt) {
        // console.log(this.model.owner, this.viewId);
        if (!this.model.owner) {
            // console.log("sending tryAcquire", this.viewId);
            this.publish(this.model.id, "tryAcquire", this.viewId);
        } else {
            if (this.viewId !== this.model.owner) {
                this.setPosition({viewId: this.model.owner, ratio: this.lastRatio});
            }
        }
        let body = scrollTarget;
        let ratio = this.getTop(body) / this.getHeight(body);
        // console.log("ratio", ratio);
        this.lastPublished.push(ratio);
        while (this.lastPublished.length > 5) {
            this.lastPublished.shift();
        }
        if (Math.abs(this.ratio - ratio) < 0.001) {return;}
        // console.log("lastPublished", this.lastPublished);
        let now = Date.now();
        // console.log("lastSetTime", this.lastSetTime, now - this.lastSetTime);
        if (now - this.lastSetTime < 50) {return;}
        this.toBePublished = ratio;
        let diff = now - this.lastPublishTime;
        // console.log("ratio", this.ratio, ratio, diff);
        if (diff < 30) {
            if (!this.timer) {
                this.timer = window.setTimeout(() => this.publishPosition(), 30);
            }
            return;
        }
        this.lastPublishTime = now;
        this.publishPosition();
    }

    publishPosition() {
        this.timer = null;
        if (this.toBePublished === null) {return;}
        // console.log("publish", this.toBePublished);
        this.publish(this.model.id, "position", {ratio: this.toBePublished, viewId: this.viewId});
        this.toBePublished = null;
    }

    handleUserCursor(data) {
        window.document.body.style.setProperty("cursor", data);
    }

    static start(opts) {
        let host = window.location.hostname;
        let path = window.location.pathname;

        let rHostParts = host.split(".").reverse();
        if (rHostParts.length === 1) {// localhost
            rHostParts = ["local.io"];
        }

        rHostParts = rHostParts.map((n) => /^[0-9]/.test(n) ? `a${n}` : n);

        let rHost = rHostParts.join(".");
        let rPath = path.split("/").filter(str => str.length > 0).reverse().join(".");
        let appId = rHost;
        if (rPath.length > 0) {
            appId += "." + rPath;
        }

        scrollTarget = document.body;
        if (document.doctype) {
            scrollTarget = document;
        }
        if (opts.scrollTarget) {
            scrollTarget = opts.scrollTarget;
            delete opts.scrollTarget;
        }

        let location = `${window.location.hostname}${window.location.pathname}${window.location.search}`;

        let options = {
            appId,
            name: location,
            model: SimplePageModel,
            view: SimplePageView,
            password: "scroll-dummy-pass",
            eventRateLimit: 60,
            tps: 0,
            ...opts
        };

        return Croquet.Session.join(options).then((session) => session.view.initCroquetMessenger());
    }
}
